...
/// Job description shared by monitor/window/region runs.
#[derive(Clone)]
...
fn adapt_frame(src_w: u32, src_h: u32, src: &[u8], dst_w: u32, dst_h: u32) -> Vec<u8> {
    ...
    T: TryInto<windows_capture::settings::GraphicsCaptureItemType>,
{
    ...
    .map_err(|e| CaptureError::Backend(format!("ffmpeg spawn: {e}")))?;
    eprintln!(
        "ffmpeg {}: {feed_w}x{feed_h}@{} -> {}x{} -> {}",
        enc.encoder_name, job.fps, canvas.width, canvas.height, job.output
    );
    // Bound 8 (~2 MB at 1080p): absorbs the WGC startup burst + encoder
    // warmup without unbounded growth; sustained backpressure still drops.
    let (tx, rx) = flume::bounded::<RawFrame>(8);
    let doc = job.annotate.clone();
    let preview_tx = job.preview_tx.clone();
    let preview_every = (job.fps.max(1) / 5).max(1) as u64;
    let cursor_fx = job.cursor_fx;
    let sample_cursor = cursor_fx.is_some();
    ...
    let pump_handle = std::thread::Builder::new()
        ...
        .map_err(|e| CaptureError::Backend(format!("pump spawn: {e}")))?;
